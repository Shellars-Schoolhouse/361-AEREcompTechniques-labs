...
then
    exr="Yes "
    pdflatex -output-directory=/dev/null -halt-on-error -interaction=batchmode report/main.tex &> /dev/null
    if [ $? -ne 0 ];
    then
...
lab_url=${lab_url:0:-4}
PR_jason=$(curl -s "https://api.github.com/repos/AERE-361/$lab_url/pulls")
